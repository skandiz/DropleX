        ...
        if (len(params['blue_particle_idx']) > 0) & (len(params['red_particle_idx']) > 0) :
            ...
            for m in range(axs.shape[1]):
                for n in range(axs.shape[0]):
                    axs[n, m].text(0.0, 1.0, f"{params['letter_labels'][i]}", transform=(axs[n, m].transAxes + ScaledTranslation(-20/72, +7/72, fig.dpi_scale_trans)), fontsize='medium', va='bottom')
                    i += 1
                    axs[n, m].set(xticks = [- 20, - 10, 0, 10, 20], yticks = [- 20, - 10, 0, 10, 20])
                    axs[n, m].set(xlim = (-20, 20), ylim = (-20, 20), xlabel = 'x [mm]', ylabel = 'y [mm]')
            ...
            if animated_plot_results:
                try:
                    ...
                    print("No rototranslation animation available, please check the parameters.")
